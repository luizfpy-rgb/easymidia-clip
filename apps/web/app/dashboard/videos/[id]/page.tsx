...
const CLIP_STATUS: Record<string, { text: string; cls: string }> = {
  suggested: { text: 'Aguardando decisão', cls: 'bg-white/10 text-mist' },
  approved: { text: 'Aprovado', cls: 'bg-emerald-950 text-emerald-300' },
  rejected: { text: 'Rejeitado', cls: 'bg-white/5 text-mist/60' },
...
export default function VideoClips() {
  ...
  useEffect(() => {
    refresh().catch(() => setMessage('Não foi possível carregar os trechos.'));
  }, [refresh]);
  ...
  // Clip que falhou na renderização volta pra fila (POST /shorts/:clipId/retry)
  async function retryRender(clipId: string) {
    setBusyClip(clipId);
    setMessage(null);
    try {
      await apiFetch(`/v1/shorts/${clipId}/retry`, { method: 'POST' });
      await refresh();
    } catch (err) {
  ...
  return (
    <AppShell>
      <Link href="/dashboard" className="text-sm text-mist hover:text-white">
        ← Voltar aos vídeos
      </Link>

      <h1 className="text-xl font-bold mt-6 mb-2">Trechos sugeridos</h1>
      ...
      {clips.length === 0 ? (
        ...
        <ul className="flex flex-col gap-4">
          {clips.map((clip) => {
            const badge = CLIP_STATUS[clip.status] ?? { text: clip.status, cls: 'bg-white/10 text-mist' };
            return (
              <li key={clip.id} className="border border-edge/60 bg-ink-2/60 rounded-lg p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    {clip.reason && <p className="text-sm text-mist mt-1">{clip.reason}</p>}
                    {clip.caption && (
                      <p className="text-sm text-mist/60 mt-2">
                        {clip.caption}{' '}
                        {clip.hashtags && (
                ...
                <div className="flex gap-2 mt-4">
                  <button
                    ...
                    className="px-4 py-2 text-sm rounded-md border border-edge hover:border-mist/50"
                  >
                  ...
                  {clip.status === 'suggested' && (
                    <>
                      ...
                      <button
                        ...
                        disabled={busyClip === clip.id}
                        className="px-4 py-2 text-sm rounded-md border border-edge hover:border-red-900 hover:text-red-300"
                      >
                        Rejeitar
                      </button>
                    </>
                  ...
                  {clip.status === 'failed' && (
                    <button
                      onClick={() => retryRender(clip.id)}
                      disabled={busyClip === clip.id}
                      className="px-4 py-2 text-sm rounded-md border border-edge hover:border-violet-500 disabled:opacity-40 font-semibold"
                    >
                      {busyClip === clip.id ? 'Enviando…' : 'Renderizar de novo'}
                    </button>
                  )}
                </div>
                ...
                {previewClip === clip.id && previewUrl && (
                  <div className="mt-4 aspect-video max-w-xl">
                    <iframe
                      src={previewUrl}
                      className="w-full h-full rounded-md border border-edge"
